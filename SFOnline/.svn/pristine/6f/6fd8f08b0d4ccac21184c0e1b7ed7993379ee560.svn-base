...
public class T200200 extends TranBase {
	...
	public void doHost( Context context ) throws SFException {
		...
		try {
			...
			String inDepName = bankUnitInCounter != null ? bankUnitInCounter.getDepName() : "";// 转出对方行行名称
			String outDepName = bankUnitOutCounter != null ? bankUnitOutCounter.getDepName() : "";// 转入对方行分行名称
			/********************************************************************
			*                    上主机记客户帐开始
			* *******************************************************************/
			...
			hostMap.put( "IN_BRANCH_ID", branchId );// 转入行号IN_BRANCH_ID
			...
			if( SFConst.RET_SUCCESS.equals( retFlag ) ) {// 交易成功
				...
				BigDecimal acctBal = signAccountData.getAcctBal().add(txAmount);
