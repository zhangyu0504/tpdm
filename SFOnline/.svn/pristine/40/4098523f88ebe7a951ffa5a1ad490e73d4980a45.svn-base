...
public class T100202 extends TranBase {
	...
	protected void initialize( Context context ) throws SFException {
		...
		try {
			...
			SFUtil.chkCond( context, SFUtil.isEmpty( capAcct ), "ST5711", String.format( "资金账号不存在" ) );
	...
	private void doCoBankReverse( Context context ) throws SFException {
		...
		try {
			...
			respMsg = SFUtil.getDataValue( context, "RESP_MSG" );
			...
			// 冲正主机返回码
			if(SFConst.RESPCODE_SUCCCODE_ESB.equals( respCode ) || SFConst.RESPCODE_ESB_REVERSESEQ_NO_EXIST.equals( respCode ) || SFConst.RESPCODE_ESB_BEEN_REVERSED.equals( respCode )){
				DBHandler.beginTransaction( context, tranConnection );// 开启事务
				...
				if( !SFConst.RET_SUCCESS.equals( retFlag ) ) {
					...
					updAgtTranList( context, tranConnection, agtTranList );
					DBHandler.commitTransaction( context, tranConnection );// 提交事务
					SFUtil.chkCond( context, "ST4527", String.format( "主机冲正成功，合作行抹帐失败[%s]", respMsg ) );
				}
				...
				SFUtil.setDataValue( context, SFConst.PUBLIC_RET_FLAG, retFlag );
				SFUtil.setDataValue( context, "RESP_CODE", respCode );
				SFUtil.setDataValue( context, "RESP_MSG", respMsg );
				SFLogger.info( context, String.format( "发送合作行冲正交易结束，返回结果：ret[%s],respCode[%s],respMsg[%s]", retFlag, respCode, respMsg ) );
			}else{
				SFUtil.chkCond( context, "ST4527", String.format( "主机抹账失败[%s]", respMsg ) );
			}
		...
		try {
			...
			if( SFConst.RET_SUCCESS.equals( retFlag ) ) {// 交易成功
				...
				if( SFUtil.isNotEmpty( jourFlag ) && "00".equals( jourFlag ) ) {
					...
					SFLogger.info( context, "更新 TRDACCTJOUR 冲正成功标志结束" );
				}
			} else { // 交易失败、超时、异常
	...
	private void updAcctJourJourFlag( Context context, String jourFlag ) throws SFException {
		...
		try {
			...
			SFUtil.chkCond( context, "ST4895", String.format( "交易流水置成冲正流水失败" ) );
		}
	...
	private void updAcctDetail( Context context, String jourFlag ) throws SFException {
		...
		try {
			...
			SFLogger.error( context, e.getMessage() );
			SFUtil.chkCond( context, "ST4895", String.format( "更新转账交易明细失败" ) );
		}
		SFLogger.info( context, "更新转账交易明细结束" );
	}
	...
	private void subAcctBalSignAccountData( Context context, Connection connection ) throws SFException {
		...
		try {
			...
			signAccountDataDao.updSignAccountData( context, tranConnection, secCompCode, capAcct, new BigDecimal( 0.00 ).subtract( txAmount ) ); // 更新签约信息表

		} catch( SFException e ) {
			throw e;
		} catch( Exception e ) {
			SFLogger.error( context, String.format( "UpdateBalFailure[%s]",e.getMessage() ) );
			SFUtil.chkCond( context, "ST4895", String.format( "更新签约表中余额失败" ) );
		}
	...
	private void addAcctBalSignAccountData( Context context, Connection connection ) throws SFException {
		...
		try {
			...
			signAccountDataDao.updSignAccountData( context, tranConnection, secCompCode, capAcct, txAmount ); // 更新签约信息表
	...
	private void updAgtTranList( Context context, Connection connection, AgtTranList agtTranList ) throws SFException {
		...
		try {
			...
			SFLogger.error( context, e.getMessage() );
			SFUtil.chkCond( context, "ST4895", String.format( "更新合作行客户交易明细失败" ) );
		}
	...
	protected void chkEnd( Context context ) throws SFException {
		try {
			...
			*************************************************/
			...
			// 是否允许券商透支当日开户控制余额
			if( "1".equals( secCompData.getOverDraftFlag() ) ) {
				if( txDate.equals( openDate ) ) {
			...
			// 被冲正流水不存在,返回券商冲正成功
			if( null == oldAcctJour || ( null != oldAcctJour && "2".equals( String.valueOf( oldAcctJour.getJourFlag().charAt( 0 ) ) ) || ( "44".equals( jourFlag ) && ( SFConst.SECU_GUOTAIJAZQ.equals( secCompCode ) || SFConst.SECU_GUOTAIJAXY.equals( secCompCode ) ) ) ) ) {
				// 置成功状态字
				succResFlag = true;
				// 组包返回券商
				doSecu( context );
			...
			SFUtil.chkCond( context, ( "44".equals( jourFlag ) && ( !SFConst.SECU_GUOTAIJAZQ.equals( secCompCode ) || !SFConst.SECU_GUOTAIJAXY.equals( secCompCode ) ) ), "ST5752", String.format( "该流水已冲正或已调整 不能冲正" ) );
			SFUtil.chkCond( context, SFUtil.isNotEmpty( oldAcctJour.getRevTxSeqId() ), "ST4502", String.format( "原流水已冲正(冲正交易)" ) );
			SFUtil.chkCond( context, ( !"1003".equals( oldAcctJour.getAbst() ) && !"2003".equals( oldAcctJour.getAbst() ) ), "ST5753", String.format( "该流水转帐类型不能被冲正" ) );
