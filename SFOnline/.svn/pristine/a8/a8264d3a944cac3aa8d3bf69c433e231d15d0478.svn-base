...
public class InvestorService {
	...
	public static void chkInvestor( Context context ) throws SFException {
		...
		try {
			...
			SFLogger.info( context, String.format( "渠道证件类型:[%s]证件号码:[%s]; 三方存管证件类型:[%s]证件号码[%s]", idType,idCode, investData.getIdType(),investData.getInvIdCode() ) );
			...
			if( SFConst.ID_TYPE_PERSON_SFZ.equals( idType ) && SFUtil.isNotEmpty(idCode) && idCode.length() == 15 ) {
				...
				SFLogger.info( context, String.format( "渠道转换18位证件号码：20世纪[%s]21世纪[%s]",invIdCode18Card19, invIdCode18Card20 ) );
				...
				SFLogger.info( context, String.format( "渠道转换15位证件号码：[%s]", invIdCode15 ) );
