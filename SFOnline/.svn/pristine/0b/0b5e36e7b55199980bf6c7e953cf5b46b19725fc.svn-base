...
public class T300102 extends TranBase {
	...
	public void doSecu( Context context ) throws SFException {
		...
		try {
			...
			String brchId = SFUtil.isNotEmpty( signAccountData.getFlags() ) ? signAccountData.getFlags() : " ";// 签约表的备注字段为机构号，如为空赋值空字符串
			  //转加密资金密码
	   	    String secAcctPwd = BizUtil.convCobankEncryptPwd(context,secCompCode,agtAgentInfo, capAcctPwd);
			// 定义全局上券商map集合，将深证通和直联所需参数放入map中
	...
	protected void chkEnd( Context context ) throws SFException {
		...
		try {
			...
			SFUtil.chkCond( context, !SFConst.SIGN_FLAG_SIGN.equals( signAccountData.getSignFlag() ), "ST5590", String.format( "客户当前状态[%s]不允许办理此交易", signAccountData.getSignFlag() ) );
