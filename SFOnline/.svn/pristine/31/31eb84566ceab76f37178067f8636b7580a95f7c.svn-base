...
public class BankSignData {
	...
	public Object[] getFieldValues() throws SFException {
		...
		for (Field f : fields) {
			try {
				if (f.getModifiers() > 2||f.get(this) instanceof Boolean || f.getName().equals("secCompName")) {
					continue;
				}
	 ...
	 * @return
	 */
	public SQLStruct getQryBankSignDataSQLStruct() throws SFException{
		StringBuffer buffer = new StringBuffer();
		Object[] valueObj = new Object[2];
		
		buffer.append("SELECT INVTYPE,ACCTID,SAVACCT,ACCTCHLDNUM,CURCODE,SECCOMPCODE,PRODUCTTYPE,CAPACCT,INVNAME,IDTYPE,INVIDCODE,SIGNFLAG,");
		...
		valueObj[0] = this.acctId;
		valueObj[1] = this.secCompCode;
		return new SQLStruct(buffer.toString(), valueObj);
	}

	public SQLStruct getSaveBankSignDataSQLStruct() throws SFException {
		StringBuffer buffer = new StringBuffer();
		Object[] valueObj = this.getFieldValues();
		
		buffer.append(" MERGE INTO TRDBANKSIGNDATA T1");
