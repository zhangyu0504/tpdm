	...
	protected byte[] getResponse( Context context ) throws Exception {
		...
		if( !context.containsKey( "A_RESPONSE_HEAD.CHCICSCODE" ) ) {
			if( SFConst.RESPCODE_SUCCCODE.equals( retCode ) || SFConst.RESPCODE_SUCCCODE_COBANK.equals( retCode ) || SFUtil.isEmpty( retCode ) ) {
				//change by 2018-05-14
				//说明：原结构为if{}  if{}else{} 导致810010永远走else逻辑
				// 现把结构改为 if{}else if{} else {}
				if("810010".equals( serviceName )){//银转证成功返回成功码为000000，不返回响应信息
					context.setDataValue( "A_RESPONSE_HEAD.RESPCODE", "000000" );
					context.setDataValue( "A_RESPONSE_HEAD.RESPMSG", "" );
				}else if( "810005".equals( serviceName ) ) {
					context.setDataValue( "A_RESPONSE_HEAD.RESPCODE", "000000" );
					context.setDataValue( "A_RESPONSE_HEAD.RESPMSG", SFUtil.isEmpty( retMsg ) ? "$" : BizUtil.returnErrMsg(context,"COBANK", retMsg) );
				} else {
					context.setDataValue( "A_RESPONSE_HEAD.RESPCODE", SFConst.RESPCODE_SUCCCODE_COBANK );
					context.setDataValue( "A_RESPONSE_HEAD.RESPMSG", SFUtil.isEmpty( retMsg ) ? SFConst.RESPCODE_SUCCMSG : BizUtil.returnErrMsg(context,"COBANK", retMsg) );
				}

				headData = ( String )headFormat.format( headColl );
			} else {
				context.setDataValue( "A_RESPONSE_HEAD.RESPCODE", retCode );
				context.setDataValue( "A_RESPONSE_HEAD.RESPMSG", BizUtil.returnErrMsg(context,"COBANK", retMsg) );
				headData = ( String )headFormat.format( headColl );
				if(!"810016".equals( serviceName ) && !"810017".equals( serviceName )){
					/*
					 * Mac 加密
					 */
		 ...
		 */
		if( !context.containsKey( "A_RESPONSE_HEAD.CHCICSCODE" ) ) {
			String macSource = SFUtil.fixChar( String.valueOf( resData.getBytes().length + 8 ), 4, '0', "left" ) + resData;
	...
	public void initHeadKcoll( Context context ) throws EMPException {
		...
		if( context.containsKey( AccessConstance.A_RESPONSE_HEAD ) ) {
			...
			if( headColl.containsKey( "CHCICSCODE" ) ) {
				...
				headColl.setDataValue( "CHMSGCODE","SC0000" );
			} else {
				if( !this.serviceName.equals( "810005" ) && !this.serviceName.equals( "810010" ) ) {// 查询签约关系和银转证响应报文不返回日期信息
					...
					if( localInfo != null ) {
						...
						// 交易为合作行的账务类对账跟客户类对账日期原路返回
						if( this.serviceName.equals( "810016" ) || this.serviceName.equals( "810017" ) ) {
							headColl.setDataValue( "BUSINESSDATE", SFUtil.getDataValue( context, "A_REQUEST_HEAD.BUSINESSDATE" ) );
							headColl.setDataValue( "TRANSDATE", SFUtil.getDataValue( context, "A_REQUEST_HEAD.TRANSDATE" ));
						}else{
			...
			if( !this.serviceName.equals( "810005" ) && !this.serviceName.equals( "810010" ) ) {// 查询签约关系和银转证响应报文不返回日期信息
				...
				if( localInfo != null ) {
					...
					// 交易为合作行的账务类对账跟客户类对账日期原路返回
					if( this.serviceName.equals( "810016" ) || this.serviceName.equals( "810017" ) ) {
						headColl.addDataField( "BUSINESSDATE", SFUtil.getDataValue( context, "A_REQUEST_HEAD.BUSINESSDATE" ) );
						headColl.addDataField( "TRANSDATE", SFUtil.getDataValue( context, "A_REQUEST_HEAD.TRANSDATE" ));
					} else {
