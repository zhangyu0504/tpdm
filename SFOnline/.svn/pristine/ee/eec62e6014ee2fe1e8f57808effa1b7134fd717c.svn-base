...
public class T300302 extends TranBase {
	...
	private void qryAcctDetail( Context context ) throws SFException {
		...
		try {
			...
			Page<AcctDetail> page = new Page<AcctDetail>( SFConst.SF_PERPAGE_NUM_COBANK, pageNum + 1 );
			page = acctDetailDao.qryAcctDetailInfo( context, tranConnection, startDate, endDate, acctId, capAcct, secCompCode, SFConst.CUR_CODE_RMB, workDate, page );
			...
			SFUtil.setResDataValue( context, "END_FLAG", page.getTotalPage() == pageNum + 1 ? "Y" : "N" );
			for( AcctDetail acctdetail : page.getPageData() ) {
				...
				SFUtil.addDataField( context, keyColl, "TX_AMOUNT", AmtUtil.conv2CoBankMulAmount( context, acctdetail.getTxAmount() ) );
