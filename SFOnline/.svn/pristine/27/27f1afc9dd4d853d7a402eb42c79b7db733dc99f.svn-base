...
public class T100301 extends TranBase {
	...
	private String acctBal = null;// 客户帐户人民币余额

	private String acctBalHKD = null;// 客户帐户港币余额

	private String acctBalUSD = null;// 客户帐户美元余额
	...
	public void doHost( Context context ) throws SFException {
		...
		DecimalFormat df = new DecimalFormat( "#0.00" );// 数字格式化，保留两位小数
		try {
			...
			if( SFConst.INIT_SIDE_COBANK.equals( initSide ) ) { // 合作行客户
				...
				/* 判断是否成功 */
				SFUtil.chkCond( context, ( !SFConst.RET_SUCCESS.equals( retFlag ) ), "ST5604", "查询银行卡余额失败" );
...
//					acctId = SFUtil.getDataValue( msgContext, kColl1, "ACCT_ID" );// 资金帐号
					...
					acctBalHKD = ( null != SFUtil.getDataValue( msgContext, kColl1, "HKD_ACCT_BAL" ) ) ? df.format( SFUtil.object2Double( SFUtil.getDataValue( msgContext, kColl1, "HKD_ACCT_BAL" ) ) ) : "0";// 港币余额
					acctBalUSD = ( null != SFUtil.getDataValue( msgContext, kColl1, "USD_ACCT_BAL" ) ) ? df.format( SFUtil.object2Double( SFUtil.getDataValue( msgContext, kColl1, "HKD_ACCT_BAL" ) ) ) : "0";// 美元余额
				...
				if( SFConst.INV_TYPE_CORP.equals( invType ) ) {
					...
					acctBalHKD = ( null != SFUtil.getDataValue( msgContext, outKColl, "HKD_ACCT_BAL" ) ) ? df.format( SFUtil.object2Double( SFUtil.getDataValue( msgContext, outKColl, "HKD_ACCT_BAL" ) ) ) : "0";// 港币余额
					acctBalUSD = ( null != SFUtil.getDataValue( msgContext, outKColl, "USD_ACCT_BAL" ) ) ? df.format( SFUtil.object2Double( SFUtil.getDataValue( msgContext, outKColl, "HKD_ACCT_BAL" ) ) ) : "0";// 美元余额
				}
	...
	public void doSecu( Context context ) throws SFException {
		...
		try {
			...
			if( SFConst.SECU_ZL.equals( secuType ) ) { // 直联模式
				...
				SFUtil.setResDataValue( context, "HKD_ACCT_BAL", String.valueOf( AmtUtil.conv2SecuMulAmount( context, acctBalHKD ) ) );// 港币余额
				SFUtil.setResDataValue( context, "USD_ACCT_BAL", String.valueOf( AmtUtil.conv2SecuMulAmount( context, acctBalUSD ) ) );// 美元余额
