...
public class PBankSqlExecService extends EMPService {
	 ...
	 */	
	public void executeSQLDef(PBankSqlDefine sqlDefInfo, Context context,
		...
		if ((sqlDefInfo.getSqlType() == PBankSqlDefine.SQL_SELECT) && !sqlDefInfo.isParaBind()) {
		...
		int nRowsBatchCommit = sqlDefInfo.getRowsBatchCommit();
		...
		try {
			...
			if (sqlDefInfo.getSqlType() == PBankSqlDefine.SQL_SELECT) //a enquiry statement
			{
				//循环绑定输入变量
				if (sqlDefInfo.isParaBind()) {
				...
				if (iCollName != null && iCollFlag == 0) // it's a IColl and circle to executeUpdate
				{
					...
					for (int j = 0; j < iColl.size(); j++) {
						...
						if (nRowsBatchCommit > 0){
							//先加入本条数据
							pState.addBatch();
							//如果达到提交的记录数，进行一次提交
							if ((j+1) % nRowsBatchCommit == 0) {
								pState.executeBatch();
							}
						}
						else {
					...
					//最后看看是否还有半截子数据，需要最后提交一次
					if (nRowsBatchCommit > 0){
						if (iColl.size() % nRowsBatchCommit != 0) {
							pState.executeBatch();
						}
					}
				}
...
//					context.setDataValue(sqlDefInfo.getRowsDataName(), Integer.toString(effectRows));
	...
	private PBankSqlDefine genNoBindSql(PBankSqlDefine sqlDefInfo, Context context) throws EMPJDBCException{
		...
		try {
			...
			if (sqlDefInfo.getSqlType() == PBankSqlDefine.SQL_SELECT) {
			...
			tmpSqlDefine.setRowsBatchCommit(sqlDefInfo.getRowsBatchCommit());
