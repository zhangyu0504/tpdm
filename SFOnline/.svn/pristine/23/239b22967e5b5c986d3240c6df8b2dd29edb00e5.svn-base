...
public class T200201 extends TranBase{
	...
	public void doHost(Context context) throws SFException {
		...
		try {
			...
			BankUnit bankUnitOutCounter = BankUnitCache.getValue( signAccountData.getOpenDepId() );// 根据开户网点号获取机构缓存对象
			String inDepName = bankUnitOutCounter != null ? bankUnitOutCounter.getDepName() : "";// 转出对方行行名称
			BankUnit bankUnitInCounter = BankUnitCache.getValue( productInfo.getTruOpnDepId() );
			String outDepName = bankUnitInCounter != null ? bankUnitInCounter.getDepName() : "";// 转入对方行分行名称
			/********************************************************************
		    *                    上主机记客户帐开始
		    ********************************************************************/
			...
			hostMap.put( "OUT_BRANCH_ID", bankUnitInCounter.getBranchId() );// 转出行号OUT_BRANCH_ID
			...
			hostMap.put( "OUT_COUNTER_CLIENT_NAME", investData.getInvName() );// 转出对方行客户名称OUT_COUNTER_CLIENT_NAME
			hostMap.put("OUT_COUNTER_BANK_NAME",SFConst.SYS_BANK_CNAME);//转出对方行银行名称OUT_COUNTER_BANK_NAME
			hostMap.put( "OUT_COUNTER_BRANCH_NAME", inDepName );// 转出对方行行名OUT_COUNTER_BRANCH_NAME
			...
			hostMap.put( "IN_COUNTER_BRANCH_NAME", outDepName );// 转入对方行分行名称IN_COUNTER_BRANCH_NAME
			...
			if (SFConst.RET_SUCCESS.equals(retFlag)) {//交易成功
				...
				acctJour.setRespMsg( String.format( "银行证转银主机交易失败[%s]",  retMsg ) );
				acctJour.setTxDate(txDate);
				acctJour.setSubTxSeqId(subTxSeqId);
				acctJourDao.saveAcctJour(context, tranConnection, acctJour);
		...
		try {
			...
			if(SFConst.SECU_SZT.equals(secuType)){//深证通方式
	 		...
	 		if (SFConst.RET_SUCCESS.equals(retFlag)) {//发券商成功
				...
				acctDetail.setBranchId( signAccountData.getOpenBranchId() );
				...
				acctJour.setRespMsg( String.format( "发券商失败[%s]", respMsg  ) );
	 			acctJour.setTxDate(txDate);
	 			acctJour.setSubTxSeqId(subTxSeqId);
	 			acctJourDao.saveAcctJour(context, tranConnection, acctJour);
	 			DBHandler.commitTransaction(context, tranConnection); //提交事务  
	...
	private void addAcctJour(Context context) throws SFException{
		...
		try {
			...
			acctJour.setBranchId( openBranch );
			...
			acctJour.setBankSeqId(BizUtil.getBankSeqid(context, chlTranSeq, initSide, invType));
