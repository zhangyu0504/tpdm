...
public class T200302 extends TranBase {
	...
	protected void chkEnd( Context context ) throws SFException {
		try {
			...
			SFUtil.chkCond( context, startDate.length() != 8 || endDate.length() != 8, "ST4012", "查询日期长度只能为8位" );
			SFUtil.chkCond( context, days > 90, "ST4012", "查询日期跨度不能超过90天" );
			...
			if( SFConst.INV_TYPE_RETAIL.equals( invType ) ) {
				...
				SFUtil.chkCond( context, null == signAccountData, "ST5720", "签约关系不存在" );
				...
				if( SFUtil.isNotEmpty( capAcct ) && SFUtil.isNotEmpty( secCompCode ) ) {// 选输参数CapAcct、SecCompCode要么都不输入，要么都要输入
					...
					SFUtil.chkCond( context, null == signAccountData, "ST5720", "资金帐号签约关系不存在" );
					if( !signAccountData.getAcctId().equals( acctId ) ) {
						SFUtil.chkCond( context, null == signAccountData, "ST5720", "传入的银行卡号与资金帐号签约的银行卡号不符" );
					}
	...
	private void qryAcctDetail( Context context ) throws SFException {
		...
		try {
			...
			SFUtil.chkCond( context, null == page || null == page.getPageData() || page.getPageData().size() == 0, "ST4069", String.format( "查询无记录" ) );
