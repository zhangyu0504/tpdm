...
public class T300201 extends TranBase {
	...
	protected void chkEnd( Context context ) throws SFException {
		...
		SFUtil.chkCond( context, !SFConst.SIGN_FLAG_SIGN.equals( signFlag ), "ST5590", "当前签约状态不允许办理此业务" );
	...
	public void doHost( Context context ) throws SFException {
		...
		if( SFConst.RET_OVERTIME.equals( retFlag ) || SFConst.RET_FAILURE.equals( retFlag ) ) {
			...
			SFUtil.chkCond( context, respCode, String.format( respMsg ) );// 主机超时失败或者合作行失败退出交易
		} else {// 合作行划款成功
	...
	public void doSecu( Context context ) throws SFException {
		...
		if( SFConst.RET_SUCCESS.equals( retFlag ) ) {// 发券商成功
			...
			SFLogger.info( context, String.format( "扣减前的AcctBal=[%s]", signAccountData.getAcctBal() ) );
			BigDecimal acctBal = signAccountData.getAcctBal().subtract( txAmount );// 交易金额
			SFLogger.info( context, String.format( "扣减后的AcctBal=[%s]", acctBal ) );
			if( "1".equals( secCompData.getOverDraftFlag() ) ) {// 该券商允许透支
				if( txDate.equals( signAccountData.getOpenDate() ) ) {// 开户当日控制余额
					DBHandler.rollBackTransaction( context, tranConnection );// 回滚事务，避免锁表
					SFUtil.chkCond( context, SFUtil.object2Double( acctBal ) < 0, "ST4113", "可用余额不足" );
				}
			} else {
				DBHandler.rollBackTransaction( context, tranConnection );// 回滚事务，避免锁表
			...
			reverse.setSceneCode( "35" );// 特殊场景码，证转银券商超时，不走后续逻辑，只要去券商冲正
	...
	private Map<String, Object> getS2BTransferMap( Context context ) throws SFException {
		...
		SFUtil.chkCond(context, SFUtil.isEmpty(secAcctPwd), "ST4223",
				String.format("证券资金密码[%s]不正确，请确认", secAcctPwd));
		// 转加密资金密码
