...
public class TransflowLimit {
	...
	public static void coBankReqFlow( Context context,String bankNo) throws SFException{
		try {
			...
			Map<String, Integer> flowLimitMap = mapTrad.get(COBANK_REQ_FLOW);//合作行请求限流缓存对象
			synchronized (flowLimitMap) {
				Integer tradReqNum = flowLimitMap.get("FLOW_LIMIT_NUM");//所有合作行请求记录数
				Integer coBankReqNum = flowLimitMap.get(bankNo);//单合作行请求记录数
				...
				flowLimitMap.put(bankNo, coBankReqNum);//增加请求记录数
				flowLimitMap.put("FLOW_LIMIT_NUM", tradReqNum);//增加总记录数
				...
				if (tradReqNum>dbFlowLimit || coBankReqNum>dbCoBankNoFlowLimit) {
					...
					SFLogger.error(context,"【重点关注】"+errMsg);
	...
	public  static void secuReqFlow( Context context,String secCompCode ) throws SFException{
		try {
			...
			Map<String, Integer> flowLimitMap = mapTrad.get(secuFlowType);//券商请求限流缓存对象
			synchronized (flowLimitMap) {
				Integer tradReqNum = flowLimitMap.get("FLOW_LIMIT_NUM");//所有券商请求记录数
				Integer secuReqNum = flowLimitMap.get(secCompCode);//单券商请求记录数
				...
				flowLimitMap.put(secCompCode, secuReqNum);//增加请求记录数
				flowLimitMap.put("FLOW_LIMIT_NUM", tradReqNum);//增加总记录数
				...
				if (SFConst.SECU_SZT.equals(sztFlag) && new Float(tradReqNum)/new Float(dbFlowLimit) >= alarmthreshold) {
					String errMsg = String.format("全【%s】券商交易请求队列数占比【%s】达到预警阀值【%s】",secuFlagName,(new Float(tradReqNum)/new Float(dbFlowLimit) * 100)+"%",(alarmthreshold * 100)+"%");
					SFLogger.error(context,"【重点关注】"+errMsg);
					...
					try {
						...
						alertLog.setTxCode(txCode);
						alertLog.setLogId(logId);
						alertLog.setType("12");//12-券商发起交易预警
						alertLog.setTxDate(DateUtil.dateToString(DateUtil.getDate(),"yyyyMMdd"));
						alertLog.setTxTime(DateUtil.dateToString(DateUtil.getDate(),"HHmmss"));
						alertLog.setMome(errMsg);
						SystemDao dao=new SystemDao();
	...
	public static Context sendCoBankFlow( Context context,String bankNo ) throws SFException{
		try {
			Map<String,Map<String,Integer>> mapTrad=CacheMap.getCache(SFConst.FLOW_CACHE_TRAD);
			Map<String, Integer> flowLimitMap = mapTrad.get(COBANK_SEND_FLOW);//调用合作行限流缓存对象
			synchronized (flowLimitMap) {
				Integer tradReqNum = flowLimitMap.get("FLOW_LIMIT_NUM");//所有合作行请求记录数
				Integer bankReqNum = flowLimitMap.get(bankNo);//单合作行请求记录数
				if(bankReqNum!=null){
				...
				flowLimitMap.put(bankNo, bankReqNum);//增加请求记录数
				flowLimitMap.put("FLOW_LIMIT_NUM", tradReqNum);//增加总记录数
