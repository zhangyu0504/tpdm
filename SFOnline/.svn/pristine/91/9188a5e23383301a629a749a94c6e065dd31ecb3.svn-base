...
public class T300100 extends TranBase {
		...
		try {
			...
			if( SFConst.SECU_SZT.equals( sztFlag ) ) {// 深证通方式
				...
				if( SFConst.RET_SUCCESS.equals( retFlag ) ) {// 深证通返回成功
					...
					secSeqId = SFUtil.getDataValue( context, refKcoll, "Ref" );// 取出券商流水号
				}
			} else {// 直联方式
				if( SFConst.SIGN_FLAG_SECU_PRE.equals( signFlag ) || SFConst.SIGN_FLAG_CONFIRM_IN_PROCESS.equals( signFlag ) ) {
				...
				/*
				 * if( SFConst.SECU_GUOTAIJAXY.equals( secCompCode )) {// 国泰证券返回 kColl = SFUtil.getDataElement( msgContext, "100100_O" ); } else { kColl = SFUtil.getDataElement( msgContext, "6022_O" ); }
				 */
				retMsg = SFUtil.getDataValue( context, kColl, "RESP_MSG" );// 直联返回错误信息
				retCode = SFUtil.getDataValue( context, kColl, "RESP_CODE" );// 直联返回错误码
				SFLogger.info( context, String.format( "券商响应码:[%s],响应信息:[%s]", retCode, retMsg ) );
	...
	protected void chkEnd( Context context ) throws SFException {
		...
		if( agtCustomerInfo != null ) {
			...
			SFUtil.chkCond( context, "0".equals( status ), "ST4393", String.format( "客户已签约" ) );
		}
		...
		// 拒绝20，21两个证件类型
		SFUtil.chkCond( context, "20".equals( idType ) || "21".equals( idType ), "ST5100", String.format( "该证件类型[%s]不允许做此业务", idType ) );
		SFUtil.chkCond( context, SFUtil.getDataValue( context, SFConst.PUBLIC_SECU ) == null, "ST5711", String.format( "券商[%s]信息不存在", secCompCode ) );
		...
		BankUnit bankUnit = BankUnitCache.getValue( depId );// 根据开户网点号获取机构缓存对象
		if( bankUnit != null ) {
		...
		if( investData == null ) {
			...
			long retailAcctNum = Long.valueOf( ParamCache.getValue( "SF_RETAIL", "CAPACCT_NUM" ).getValue() );// 零售客户最大签约数
			long corpAcctNum = Long.valueOf( ParamCache.getValue( "SF_CORP", "CAPACCT_NUM" ).getValue() );// 机构客户最大签约数
			...
			if( SFConst.INV_TYPE_RETAIL.equals( invType ) ) {// 零售客户
				...
				SFUtil.chkCond( context, ( ( capAcctCount == 0 && secAcctCount > sfCapAcctNum ) || ( capAcctCount != 0 && secAcctCount > retailAcctNum ) ), "ST5100", String.format( "个人客户最多可以同时签约[%s]个资金账号", sfCapAcctNum ) );
			} else {// 机构客户
				sfCapAcctNum = Long.valueOf( ParamCache.getValue( "SF_CORP", "CAPACCT_NUM" ).getValue() );
				SFUtil.chkCond( context, ( ( capAcctCount == 0 && secAcctCount > sfCapAcctNum ) || ( capAcctCount != 0 && secAcctCount > corpAcctNum ) ), "ST5100", String.format( "机构客户最多可以同时签约[%s]个资金账号", sfCapAcctNum ) );
			}
		}
	}
	...
	private AcctJour getAcctJour( Context context ) throws SFException {
		...
		acctJour.setPreSeqId( logId );// 流水号改造，跟AcctDealId保持一致
		acctJour.setAcctDealId( logId );// 需要跟对账保持一致，取后14位
