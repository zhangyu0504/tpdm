...
public abstract class TranBase {
	 ...
	 * @throws SFException
	 */
	private void chkSecu( Context context ) throws SFException {
		...
		try {
			...
			if( SFUtil.isEmpty( secCompCode ) ) {
				...
				SFUtil.setDataValue( context, SFConst.PUBLIC_SEC_SERV_STATUS, null );
			} else {
				...
				SecServStatus secServStatus = secServStatusDao.qrySecServStatus( context, tranConnection, secCompCode, "03" );
				SFUtil.setDataValue( context, SFConst.PUBLIC_SEC_SERV_STATUS, secServStatus );
			}
			...
			if( '1' == secuCtlFlags.charAt( 0 ) ) {// 检查系统状态
				SecuService.chkSubCenterStatus( context );
			}
			if( '1' == secuCtlFlags.charAt( 1 ) || '1' == secuCtlFlags.charAt( 2 ) || '1' == secuCtlFlags.charAt( 3 ) ) {
				if( '1' == secuCtlFlags.charAt( 1 ) && !chkFlag ) {
					chkFlag = SecuService.chkSecuStatus( context, tranConnection ); // 检查系统停启状态
				}
				if( '1' == secuCtlFlags.charAt( 2 ) && !chkFlag ) {
					chkFlag = SecuService.chkSecu724( context, tranConnection );// 检查是否允许进行724交易
				}
				if( '1' == secuCtlFlags.charAt( 3 ) && !chkFlag ) {
					chkFlag = SecuService.chkSecuHoliday( context, tranConnection );// 检查是否允许进行节假日交易
				}

				SFUtil.chkCond( context, !chkFlag, "ST4431", "非正常交易时间不允许做此交易" );
			}
			if( '1' == secuCtlFlags.charAt( 4 ) ) {// 检查账户类服务标志
				SecuService.chkSecuAcctFlag( context, tranConnection );
			}
			if( '1' == secuCtlFlags.charAt( 5 ) ) {// 检查单户结息服务标志
				SecuService.chkSecuAccrualFlag( context );
			}
			if( '1' == secuCtlFlags.charAt( 6 ) ) {// 检查是否允许银行发起银转证
				SecuService.chkSecuBankChlB2S( context );
			}
			if( '1' == secuCtlFlags.charAt( 7 ) ) {// 检查是否允许银行发起证转银
				SecuService.chkSecuBankChlS2B( context );
			}
			if( '1' == secuCtlFlags.charAt( 8 ) ) {// 检查是否允许券商发起银转证
				SecuService.chkSecuChlB2S( context );
			}
			if( '1' == secuCtlFlags.charAt( 9 ) ) {// 检查是否允许券商发起证转银
				SecuService.chkSecuChlS2B( context );
			}
			if( '1' == secuCtlFlags.charAt( 10 ) ) {// 是否允许办理该币种交易
	 ...
	 * @throws SFException
	 */
	private void chkInvestor( Context context ) throws SFException {
		try {
			String secCompCode = SFUtil.getReqDataValue( context, "SEC_COMP_CODE" );// 从上下文取出券商代码
			...
			// change by songshimin 20180424-16:53
			// 说明:将证件类型转换的代码位置从209行出移到此处
			// 目的：证件类型转换需要用到证件号码，证件号码需从PUBLIC_INVEST_DATA对象取出，故要先在Context中放入PUBLIC_INVEST_DATA对象
			...
			if( SFConst.INIT_SIDE_COBANK.equals( initSide ) ) {// 合作行渠道
				...
				bankId = SFUtil.getReqDataValue( context, "BANK_ID" );// 其他渠道从报文体中取出BankId
			}
			...
			if( SFUtil.isEmpty( bankId ) ) {
				...
				SFUtil.setDataValue( context, SFConst.PUBLIC_AGENTINFO, agtAgentInfoDao.qryAgtAgentInfo( context, tranConnection, bankId ) );
			}
			String txCode = SFUtil.getDataValue( context, SFConst.PUBLIC_TX_CODE );// 交易码
