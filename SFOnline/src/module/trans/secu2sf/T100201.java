 ...
 * 
 * 处理流程：
 *  		1.接收券商交易 （<深>Trf.001.01/12002，<直>6041/612204）
 *  		2.检查请求包有效性,检查券商信息,客户信息有效性,检查资金帐户余额等
 *			3.代理：调用612322转发代理平台    非代理：调用G1001转账（券商汇总账户到客户帐）
 *			4.返回券商 （<深>Trf.002.01/12002，<直>6041/612204）
 *	
 *  交易码：100201
 * @author 许张鸿达
 * 
...
public class T100201 extends TranBase {
	...
	private String tpdmReverse = "NONE";// 添加冲正任务方 NONE-主机失败，不冲主机 HOST-主机超时或者合作行失败，已经添加冲正主机任务
	...
	protected void initialize( Context context ) throws SFException {
		...
		try {
			...
			SFUtil.chkCond( context, SFUtil.isEmpty( secCompCode ), "ST5711", String.format( "券商编号不允许为空" ) );
	...
	protected void chkEnd( Context context ) throws SFException {
		...
		// 检查券商信息是否存在
		SFUtil.chkCond( context, ( null == secCompData || SFUtil.isEmpty( secCompData.getSecCompCode() ) ), "ST5705", String.format( "券商代码错误" ) );
		...
		if( "1".equals( secCompData.getOverDraftFlag() ) ) {
			if( txDate.equals( signAccountData.getOpenDate() ) ) {
	...
	public void doHost( Context context ) throws SFException {
		...
		try {
			...
			BankUnit bankUnitOutCounter = BankUnitCache.getValue( signAccountData.getOpenBranchId() );// 根据开户网点号获取机构缓存对象
			BankUnit bankUnitInCounter = BankUnitCache.getValue( productInfo.getTruOpnDepId() );// 根据开户网点号获取机构缓存对象
			...
			if( SFConst.RET_SUCCESS.equals( retFlag ) ) { // 成功
				...
				acctJour.setRespMsg( String.format( "上主机交易异常[%s]",retMsg ) );
			...
			// 上主机失败，回置金额并更改状态
			if( SFConst.RET_FAILURE.equals( retFlag ) && "NONE".equals( tpdmReverse ) ) {
				acctDetailDao.saveAcctDetail( context, tranConnection, acctDetail ); // 更新交易明细状态
				updSignAccountData( context, txAmount ); // 更新签约信息表
			}
	...
	public void addPublicInfo( Context context ) throws SFException {
		...
		try {
			...
			updSignAccountData( context, new BigDecimal( 0.00 ).subtract( txAmount ) ); // 更新签约信息表
	...
	private void updSignAccountData( Context context, BigDecimal txAmount ) throws SFException {
	...
	private String addTranList( Context context, String tranType ) throws SFException {
		...
		try {
			...
			if( agtCustomerInfo == null ) {
				SFLogger.error( context, String.format( "合作行客户[%s]不存在，请检查", stkAcct ) );
			...
			SFLogger.error( context, String.format( "addTranList()处理失败%s", e.getMessage() ) );
			return SFConst.RET_FAILURE;
		} catch( Exception e ) {
			SFLogger.error( context, String.format( "addTranList()处理失败%s", e.getMessage() ) );
			return SFConst.RET_FAILURE;
	...
	public String cobankS2BTransfer( Context context ) throws SFException {
		...
		String ret = addTranList( context, "1" );
		if( !ret.equals( SFConst.RET_SUCCESS ) ) {
			return ret;
		}
		String secCompCode = SFUtil.getReqDataValue( context, "SEC_COMP_CODE" );// 券商代码
		String stkAcct = SFUtil.getReqDataValue( context, "CAP_ACCT" );// 证券资金帐号
		...
		try {
			...
			retFlag = SFUtil.getDataValue( msgContext, SFConst.PUBLIC_RET_FLAG );
			String retCode = null;
			if( SFConst.RET_FAILURE.equals( retFlag ) || SFConst.RET_OVERTIME.equals( retFlag ) ) {
				IndexedCollection iColl = SFUtil.getDataElement( msgContext, "RET" );
				KeyedCollection kColl = ( KeyedCollection )iColl.getElementAt( 0 );// 获取响应数组
				retCode = SFUtil.getDataValue( msgContext, kColl, "RET_CODE" );
