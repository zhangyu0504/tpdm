...
public class T200106 extends TranBase {
	...
	private String branchId = null;// 开户机构号
	
	private String openDepId = null;// 开户网点号

	private String initSeqId = null;// 日志号

	private String subTxSeqId = null;// 流水号

	private String tranSeqId = null;// 22位主机流水号
	...
	public void doHost( Context context ) throws SFException {
		...
		try {
			...
			***************************************************************************/
			...
			SFLogger.info( context, String.format( "卡类型2为联名卡，1为普卡,卡预约号[%s],卡类型[%s]-卡号[%s]对应的签约状态[%s]", bookNo, cardType, acctId, signFlag ) );
	...
	public void doSecu( Context context ) throws SFException {
		...
		try {
			if( "1".equals( cardType ) && "2".equals( secCompData.getBankPreSignMode2() ) ) {// 普通卡实时发券商处理
				...
				secuMap.put( "BRANCH_ID", branchId );// 上送机构号
				secuMap.put( "OPEN_DEP_ID", openDepId );// 上送开户网点Dgst
