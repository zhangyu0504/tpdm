...
public class T200102 extends TranBase {
	...
	private String secSeqId = null; // 发券商返回的流水号

	private String becifNo = null; // 发券商返回的流水号
	...
	protected void initialize( Context context ) throws SFException {
		...
		if (SFConst.INV_TYPE_CORP.equals(invType)) {// 对公客户才获取交易网点
	...
	public void doHost( Context context ) throws SFException {
		...
		try {
			...
			SFUtil.chkCond( context, ( null == signAccountData ), "ST5720", "签约信息不存在" );
			...
			brchId = SFUtil.isNotEmpty( signAccountData.getFlags() ) ? signAccountData.getFlags() : " ";// 签约表的备注字段为机构号，如为空赋值空字符串
			...
			SFUtil.chkCond( context, ( SFConst.INIT_SIDE_COBANK.equals( initSideFlag ) ) && SFUtil.isEmpty( chOpenNode ), "ST5771", "非本行客户不允许从本行渠道发起变更" );

			BankUnit bankUnit = BankUnitCache.getValue(chNode);// 根据开户网点号获取机构缓存对象
			// 个人户bankUnit允许为空
			SFUtil.chkCond(context, SFConst.INV_TYPE_CORP.equals(invType)
					&& bankUnit == null, "ST5801", "查询开户网点号所属分行失败");
			if (bankUnit != null) {
				chBranch = bankUnit.getBranchId();// 所属分行号
			}
			...
			if( SFConst.INV_TYPE_RETAIL.equals( invType ) ) // 个人
			{
				...
				//说明:客户姓名、证件类型拿请求报文的值来比较；不需要拿数据库的证件号码进行比较，用渠道的比较即可
				//SFUtil.chkCond( context, !signAccountData.getInvName().equals( chHostName ), "ST4090", "客户账户名称不符" );// 取客户账号名称，不取客户名称
				//SFUtil.chkCond( context, !investData.getIdType().equals( chHostType ), "ST4090", "客户证件类型不符" );
				//SFUtil.chkCond( context, !investData.getInvIdCode().equals( chHostCode ), "ST4090", "客户证件号码不符" );
				SFUtil.chkCond( context, !invName.equals( chHostName ), "ST4090", "客户账户名称不符" );// 取客户账号名称，不取客户名称
				SFUtil.chkCond( context, !idType.equals( chHostType ), "ST4090", "客户证件类型不符" );
				//Change By zhangyu 2018-7-4 End

				
				SFUtil.chkCond( context, ( !IdCode_18.equals( chHostCode_18 ) ) && ( !IdCode_18.equals( chHostCode_18_20 ) ) && ( !InvIdCode18_20.equals( chHostCode_18_20 ) && ( !chHostCode_18.equals( InvIdCode18_20 ) ) ), "ST4090", "客户身份信息核对不符" );
				...
				// 说明 ：对公户从C3011响应取出openDepId 所属机构号
				...
				String secuTrustIDType = SFUtil.getDataValue( QueryMsgContext, "MSG_O.STOCKJOBBER_TRUST_GLOBAL_TYPE" );// C3011接口返回券商托管证件类型
				String secuTrust2SFIdType = BizUtil.convHost2SF4IdType( context, secuTrustIDType );// 主机证件类型转三方证件类型
				String secuTrustIDCode = SFUtil.getDataValue( QueryMsgContext, "MSG_O.STOCKJOBBER_TRUST_GLOBAL_NO" );// C3011接口返回券商托管证件号码
				...
				if( "80".equals( NewchHostType ) ) {
					...
					String hostRetIdType = SFUtil.getDataValue( msgContext, "MSG_O.OTHER_PROVE_FILE_TYPE" );// M8010接口返回同业证件类型
					hostRetIdType = BizUtil.convHost2SF4IdType( msgContext, hostRetIdType );// 主机证件类型转三方证件类型
					String hostRetIdCode = SFUtil.getDataValue( msgContext, "MSG_O.OTHER_PROVE_FILE_NO" );// M8010接口返回同业证件号码
					SFUtil.chkCond( context, ( !investData.getIdType().equals( hostRetIdType ) || !investData.getInvIdCode().equals( hostRetIdCode ) ) && ( !investData.getIdType().equals( secuTrust2SFIdType ) || !investData.getInvIdCode().equals( secuTrustIDCode ) ), "ST4090", "证件不符" );
					idType = hostRetIdType;

					SFLogger.info( context, String.format( "同业对公客户信息核对结束" ) );
				} else {
					SFUtil.chkCond( context, ( !investData.getIdType().equals( NewchHostType ) || !investData.getInvIdCode().equals( NewchHostCode ) ) && ( !investData.getIdType().equals( secuTrust2SFIdType ) || !investData.getInvIdCode().equals( secuTrustIDCode ) ), "ST4090", "证件不符" );
				}
				...
				// SFUtil.chkCond(context,!signAccountData.getInvName().equals(NewchHostName), "ST4090", "客户账户名称不符");//取客户账号名称，不取客户名称
				// SFUtil.chkCond(context,!investData.getIdType().equals(NewchHostType), "ST4090", "客户证件类型不符");
				// SFUtil.chkCond(context,!investData.getInvIdCode().equals(NewchHostCode), "ST4090", "客户证件号码不符");
			...
			BizUtil.setCardStatusWord( context, newAcctId, "A", chOpenNode );// 功能码 A-建立
	...
	public void doSecu( Context context ) throws SFException {
		...
		try {
			...
			BizUtil.setSecBrchId( secCompCode, msg, brchId );//银河、五矿需送机构号
			...
			if( SFConst.SECU_SZT.equals( sztFlag ) ) {
				...
				retCode = SFUtil.getDataValue(context, kColl, "RESP_CODE");// 直连返回错误码
			...
			if( SFConst.RET_FAILURE.equals( retFlag ) ) {
				...
				if( null == list || list.size() == 0 ) {
					...
					BizUtil.setCardStatusWord( context, newAcctId, "D", chOpenNode );// 功能码D-取消
				...
				DBHandler.beginTransaction( context, tranConnection );// 开启事务
				acctJourDao.updAcctJourDataByErr(context, tranConnection,
						retCode, "发送券商超时异常", tranDate, "6024", subTxSeqId);
				DBHandler.commitTransaction( context, tranConnection );// 提交事务
			...
			} else // 成功
			{
				...
				if( invType.equals( SFConst.INV_TYPE_RETAIL ) ) {// 个人
					...
					try {
						...
						/*********************** 成功签约的数据插入到表TRDAUTOBECIF中，后续轮询发协议到BECI结束 *******************************/
					} catch( Exception e ) {// BECIF发送失败，不影响交易
				...
				if( DBcount == 0 ) {
					try {
						...
						msgMap.put( "BRANCH_ID", chOpenNode );// 开户网点号
						...
						SFLogger.error( context, "取消旧卡的状态字失败,不影响交易流程!" );
					}
			...
			// 交易成功，组返回报文
			SFUtil.setResDataValue( context, "OLD_ACCT_ID", oldAcctId );// 原卡号
			SFUtil.setResDataValue( context, "NEW_ACCT_ID", newAcctId );// 新卡号
