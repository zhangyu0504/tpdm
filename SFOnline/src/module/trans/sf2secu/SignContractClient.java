...
public class SignContractClient extends SecuClientBase {
	...
	protected Context doHandle( Context context, Map<String, Object> msg ) throws SFException {
		...
		SFLogger.info( context, String.format( "券商证件类型转存管证件类型成功，返回:[%s] ", idType ) );
	...
	private Context szt11001( Context context, Map<String, Object> msg ) throws SFException {
		SFLogger.info( context, "发送报文至深证通客户指定存管银行开始" );
		// 克隆深证通通信临时上下文
		...
		SFUtil.setDataValue( msgContext, custKcoll, "ADDR", msg.get( "ADDR" ) );// Cust/CertId
		SFUtil.setDataValue( msgContext, custKcoll, "ZIP", msg.get( "ZIP" ) );// Cust/CertId
		SFUtil.setDataValue( msgContext, custKcoll, "EMAIL_ADDR", msg.get( "EMAIL_ADDR" ) );// Cust/CertId
		SFUtil.setDataValue( msgContext, custKcoll, "FAX", msg.get( "FAX" ) );// Cust/CertId
		SFUtil.setDataValue( msgContext, custKcoll, "MOBILE", msg.get( "MOBILE" ) );// Cust/CertId
		SFUtil.setDataValue( msgContext, custKcoll, "PHONE", msg.get( "PHONE" ) );// Cust/CertId
		// 客户类型转换
		...
		SFLogger.info( msgContext, "发送报文至深证通客户指定存管银行结束" );
		return msgContext;
	}
	...
	private Context szt11003( Context context, Map<String, Object> msg ) throws SFException {
		...
		if( SFUtil.isNotEmpty( msg.get( "SEC_BRCH_ID" ) ) ) {
			...
			SFUtil.setDataValue( msgContext, "MsgHdr.Recver.BrchId", msg.get( "SEC_BRCH_ID" ) );
			/*
			 * KeyedCollection headKcoll = new KeyedCollection("MsgHdr"); KeyedCollection recverKcoll = new KeyedCollection("Recver"); SFUtil.addDataField( context, recverKcoll, "BrchId", msg.get( "SEC_BRCH_ID" ) ); SFUtil.addDataElement( context, headKcoll, recverKcoll ); SFUtil.addDataElement( msgContext, headKcoll );
			 */
		}
		SFLogger.info( context, String.format( "券商证件类型转存管证件类型成功，返回:[%s] ", msg.get( "ID_TYPE" ) ) );
		KeyedCollection custKcoll = SFUtil.getDataElement( msgContext, kColl, "Cust" );
		SFUtil.setDataValue( msgContext, custKcoll, "INV_NAME", msg.get( "INV_NAME" ) );// Cust/Name
		SFUtil.setDataValue( msgContext, custKcoll, "ID_TYPE_SZT", msg.get( "ID_TYPE" ) );// Cust/CertType
		SFUtil.setDataValue( msgContext, custKcoll, "INV_ID_CODE", msg.get( "INV_ID_CODE" ) );// Cust/CertId
