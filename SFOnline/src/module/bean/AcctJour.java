...
public class AcctJour {
	...
	private int abnDealTimes;
	private String abnDealTxTime;
	private String memo;
	private String respCode;
	private String respMsg;
	private String tranSeqId;
	private String busiSeqId;
	private String chlSeqId;
	private String chlTranSeqId;
	private String UMId;
	...
	private boolean revTxSeqIdChanged = false;
	...
	private boolean oldIdTypeChanged = false;
	...
	private boolean bankSeqIdChanged = false;
	private boolean productTypeChanged = false;
	private boolean colFlagChanged = false;
	private boolean abortFlagChanged = false;
	private boolean abnDealTimesChanged = false;
	private boolean abnDealTxTimeChanged = false;
	private boolean memoChanged = false;
	private boolean respCodeChanged = false;
	private boolean respMsgChanged = false;
	private boolean tranSeqIdChanged = false;
	private boolean busiSeqIdChanged = false;
	private boolean chlSeqIdChanged = false;
	private boolean chlTranSeqIdChanged = false;
	private boolean UMIdChanged = false;
	private boolean ybStatusChanged = false;
	...
	public String getRevTxSeqId() {
		return revTxSeqId;
	}

	public void setRevTxSeqId(String revTxSeqId) {
		this.revTxSeqId = revTxSeqId;
		this.revTxSeqIdChanged = true;
	}
	...
	public void setAbortFlag(String abortFlag) {
		...
		this.abortFlagChanged = true;
	}

	public int getAbnDealTimes() {
		return abnDealTimes;
	}

	public void setAbnDealTimes(int abnDealTimes) {
		this.abnDealTimes = abnDealTimes;
		this.abnDealTimesChanged = true;
	}

	public String getAbnDealTxTime() {
		return abnDealTxTime;
	}

	public void setAbnDealTxTime(String abnDealTxTime) {
		this.abnDealTxTime = abnDealTxTime;
		this.abnDealTxTimeChanged = true;
	}
	...
	public String getChlSeqId() {
		return chlSeqId;
	}

	public void setChlSeqId(String chlSeqId) {
		this.chlSeqId = chlSeqId;
		this.chlSeqIdChanged = true;
	}

	public String getChlTranSeqId() {
		return chlTranSeqId;
	}

	public void setChlTranSeqId(String chlTranSeqId) {
		this.chlTranSeqId = chlTranSeqId;
		this.chlTranSeqIdChanged = true;
	}
	 ...
	 * @throws SFException
	 */
	public SQLStruct getSaveAcctJourSQLStruct(String tableName) throws SFException {
		StringBuffer buffer = new StringBuffer();
		Object[] valueObj = this.getFieldValues();
		
		buffer.append("MERGE INTO " + tableName + " T1");
		buffer.append(" USING (SELECT ? as TXDATE, ? as INITSIDE, ? as USERID, ? as TXSEQID, ? as REVTXSEQID, ? as SECSEQID, ? as SUBTXSEQID, ? as INVTYPE,");
		buffer.append(" ? as INVNAME, ? as IDTYPE, ? as INVIDCODE, ? as OLDINVNAME, ? as OLDIDTYPE, ? as OLDINVIDCODE, ? as SECACCT, ? as SECACCTSEQ, ? as SECCOMPCODE,");
		buffer.append(" ? as CAPACCT, ? as ACCTID, ? as SAVACCT, ? as OPENDEPID, ? as OPENBRANCHID, ? as OLDACCTID, ? as CURCODE, ? as DCFLAG, ? as TXAMOUNT, ? as ACCTBAL,");
		buffer.append(" ? as ABSTRACT, ? as ABSTRACTSTR, ? as JOURFLAG, ? as TXCODE, ? as BUSITYPE, ? as TXTIME, ? as DEPID, ? as BRANCHID, ? as UNITTELLERID, ? as CASHREMITFLAG,");
		buffer.append(" ? as ACCTDEALID, ? as ACCTHOSTSEQID, ? as PRESEQID, ? as ACCTDEALDATE, ? as BANKSEQID, ? as PRODUCTTYPE, ? as COLFLAG, ? as ABORTFLAG, ? as ABNDEALTIMES,");
		buffer.append(" ? as ABNDEALTXTIME, ? as MEMO, ? as RESPCODE, ? as RESPMSG, ? as TRANSEQID, ? as BUSISEQID, ? as CHLSEQID, ? as CHLTRANSEQID, ? as UMID, ? as YBSTATUS");
		...
		if (revTxSeqIdChanged) {
			buffer.append("T1.REVTXSEQID = T2.REVTXSEQID,");
		}
		if (secSeqIdChanged) {
		...
		if (bankSeqIdChanged) {
			buffer.append("T1.BANKSEQID = T2.BANKSEQID,");
		}
		...
		if (abortFlagChanged) {
			buffer.append("T1.ABORTFLAG = T2.ABORTFLAG,");
		}
		if (abnDealTimesChanged) {
			buffer.append("T1.ABNDEALTIMES = T2.ABNDEALTIMES,");
		}
		if (abnDealTxTimeChanged) {
			buffer.append("T1.ABNDEALTXTIME = T2.ABNDEALTXTIME,");
		}
		if (memoChanged) {
			buffer.append("T1.MEMO = T2.MEMO,");
		}
		if (respCodeChanged) {
			buffer.append("T1.RESPCODE = T2.RESPCODE,");
		}
		if (respMsgChanged) {
			buffer.append("T1.RESPMSG = T2.RESPMSG,");
		}
		if (tranSeqIdChanged) {
		...
		if (chlSeqIdChanged) {
			buffer.append("T1.CHLSEQID = T2.CHLSEQID,");
		}
		if (chlTranSeqIdChanged) {
			buffer.append("T1.CHLTRANSEQID = T2.CHLTRANSEQID,");
		}
		if (UMIdChanged) {
			buffer.append("T1.UMID = T2.UMID,");
		}
		if (ybStatusChanged) {
			buffer.append("T1.YBSTATUS = T2.YBSTATUS");
		}
		if (buffer.substring(buffer.length() - 1).equals(",")) {
			buffer = buffer.deleteCharAt(buffer.length() - 1);
		}
		buffer.append(" WHEN NOT MATCHED THEN");
		buffer.append(" insert(TXDATE,INITSIDE,USERID,TXSEQID,REVTXSEQID,SECSEQID,SUBTXSEQID,INVTYPE,INVNAME,IDTYPE,INVIDCODE,OLDINVNAME,OLDIDTYPE,");
		buffer.append("OLDINVIDCODE,SECACCT,SECACCTSEQ,SECCOMPCODE,CAPACCT,ACCTID,SAVACCT,OPENDEPID,OPENBRANCHID,OLDACCTID,CURCODE,DCFLAG,TXAMOUNT,");
		buffer.append("ACCTBAL,ABSTRACT,ABSTRACTSTR,JOURFLAG,TXCODE,BUSITYPE,TXTIME,DEPID,BRANCHID,UNITTELLERID,CASHREMITFLAG,ACCTDEALID,ACCTHOSTSEQID,");
		buffer.append("PRESEQID,ACCTDEALDATE,BANKSEQID,PRODUCTTYPE,COLFLAG,ABORTFLAG,ABNDEALTIMES,ABNDEALTXTIME,MEMO,RESPCODE,RESPMSG,TRANSEQID,BUSISEQID,");
		buffer.append("CHLSEQID,CHLTRANSEQID,UMID,YBSTATUS)");
		buffer.append(" values(T2.TXDATE,T2.INITSIDE,T2.USERID,T2.TXSEQID,T2.REVTXSEQID,T2.SECSEQID,T2.SUBTXSEQID,T2.INVTYPE,T2.INVNAME,T2.IDTYPE,T2.INVIDCODE,");
		buffer.append("T2.OLDINVNAME,T2.OLDIDTYPE,T2.OLDINVIDCODE,T2.SECACCT,T2.SECACCTSEQ,T2.SECCOMPCODE,T2.CAPACCT,T2.ACCTID,T2.SAVACCT,T2.OPENDEPID,T2.OPENBRANCHID,");
		buffer.append("T2.OLDACCTID,T2.CURCODE,T2.DCFLAG,T2.TXAMOUNT,T2.ACCTBAL,T2.ABSTRACT,T2.ABSTRACTSTR,T2.JOURFLAG,T2.TXCODE,T2.BUSITYPE,T2.TXTIME,T2.DEPID,T2.BRANCHID,");
		buffer.append("T2.UNITTELLERID,T2.CASHREMITFLAG,T2.ACCTDEALID,T2.ACCTHOSTSEQID,T2.PRESEQID,T2.ACCTDEALDATE,T2.BANKSEQID,T2.PRODUCTTYPE,T2.COLFLAG,T2.ABORTFLAG,");
		buffer.append("T2.ABNDEALTIMES,T2.ABNDEALTXTIME,T2.MEMO,T2.RESPCODE,T2.RESPMSG,T2.TRANSEQID,T2.BUSISEQID,T2.CHLSEQID,T2.CHLTRANSEQID,T2.UMID,T2.YBSTATUS)");
	...
	public void resetChangedFlag() {
		...
		this.bankSeqIdChanged = false;
		this.productTypeChanged = false;
		this.colFlagChanged = false;
		this.abortFlagChanged = false;
		this.abnDealTimesChanged = false;
		this.abnDealTxTimeChanged = false;
		this.memoChanged = false;
		this.respCodeChanged = false;
		this.respMsgChanged = false;
		this.tranSeqIdChanged = false;
		this.busiSeqIdChanged = false;
		this.chlSeqIdChanged = false;
		this.chlTranSeqIdChanged = false;
		this.UMIdChanged = false;
		this.ybStatusChanged = false;
	}
