...
public class InvestDataDao extends DaoBase{
	...
	private StringBuffer getQryInvestDataSQLStruct(){
		...
		buffer.append(" FROM TRDINVESTDATA WHERE  ");
	...
	public InvestData qryInvestData(Context context,Connection connection,String idType,String invIdCode) throws SFException{
		...
		try {
			...
			buffer.append(" IDTYPE = ? AND INVIDCODE = ?");
			
			investData = super.qry(context, connection, buffer.toString(), InvestData.class, idType , invIdCode);
		} catch (SFException e){
			throw new SFException(e);
		} catch (Exception e ){
			SFUtil.chkCond(context, "ST4895", e.getMessage());
	...
	public InvestData qryInvestDataByInvType(Context context,Connection connection,String idType,String invIdCode,String invType) throws SFException{
		...
		try {
			StringBuffer buffer = getQryInvestDataSQLStruct();
			buffer.append(" IDTYPE = ? AND INVIDCODE = ? AND INVTYPE = ?");
			
			investData = super.qry(context, connection, buffer.toString(), InvestData.class, idType , invIdCode, invType);
		} catch (SFException e){
			throw new SFException(e);
		} catch (Exception e ){
			SFUtil.chkCond(context, "ST4895", e.getMessage());
	...
	public InvestData qryInvestDataBySecAcct(Context context,Connection connection,String secAcct) throws SFException {
		...
		try {
			StringBuffer buffer = getQryInvestDataSQLStruct();
			buffer.append(" SECACCT = ?");
			investData = super.qry(context, connection, buffer.toString(), InvestData.class,secAcct);
		} catch (SFException e){
			throw new SFException(e);
		} catch (Exception e ){
			SFUtil.chkCond(context, "ST4895", e.getMessage());
	 ...
	 * @throws SFException
	 */
	public InvestData qryInvestDataByInvIdCode(Context context,Connection connection,String idType,String invIdCode,String convInvIdCode1,String convInvIdCode2) throws SFException{
		InvestData investData = null;
		 try {
			 
			 StringBuffer buffer = getQryInvestDataSQLStruct();
			 
			 buffer.append(" IDTYPE =? AND INVIDCODE IN(?,?,?)");
				 
			 investData = super.qry(context, connection, buffer.toString(),InvestData.class, idType,invIdCode,convInvIdCode1,convInvIdCode2);
