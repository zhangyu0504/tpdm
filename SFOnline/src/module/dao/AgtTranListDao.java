...
public class AgtTranListDao extends DaoBase {
	...
	private StringBuffer getQryAgtTranListSQLStruct(){
		...
		buffer.append(" FROM AGT_TRANLIST WHERE ");
	...
	public AgtTranList qryAgtTranListBylogNo(Context context,Connection connection,String queryLogNo)throws SFException{
		...
		try{
			...
			buffer.append(" PICSLOGNO=? OR FRONTLOGNO=? OR AGENTLOGNO=? ");
			
		    agtTranList = super.qry(context, connection, buffer.toString(), AgtTranList.class, queryLogNo,queryLogNo,queryLogNo);
		} catch (SFException e){
			throw e;
		} catch (Exception e ){
			SFUtil.chkCond(context, "ST4895", e.getMessage());
	...
	public AgtTranList qryAgtTranListByBankIdAndQueryLogNo(Context context,Connection connection,String bankId,String queryLogNo) throws SFException{
		...
		try {
			StringBuffer buffer = getQryAgtTranListSQLStruct();
			buffer.append(" BANKID=? AND (PICSLOGNO=? OR AGENTLOGNO=?)");
			agtTranList = super.qry(context, connection, buffer.toString(), AgtTranList.class, bankId,queryLogNo,queryLogNo);
		} catch (SFException e){
			throw e;
		} catch (Exception e ){
			SFUtil.chkCond(context, "ST4895", e.getMessage());
	...
	public AgtTranList qryAgtTranListByBankId(Context context,Connection connection,String businessType,String PICSLogno)throws SFException{
		...
		try {
			StringBuffer buffer = getQryAgtTranListSQLStruct();
			buffer.append(" BUSINESSTYPE = ? AND PICSLOGNO = ? ");
			buffer.append(" AND FRONTLOGNO IN (SELECT MAX(FRONTLOGNO) FROM AGT_TRANLIST WHERE BUSINESSTYPE = ? AND PICSLOGNO = ?)");
			agtTranList = super.qry(context,connection,buffer.toString(),AgtTranList.class,businessType,PICSLogno,businessType,PICSLogno);
		} catch (SFException e){
			throw e;
		} catch (Exception e ){
			SFUtil.chkCond(context, "ST4895", e.getMessage());
