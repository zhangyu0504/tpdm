...
public class PBankTCPIPDataSerializerService extends PBankTCPIPRequestService {
	...
	protected byte[] getResponse(Context context, DataElement dataElement) throws Exception {
		...
		if(SFUtil.isEmpty(retCode)||SFConst.RESPCODE_SUCCCODE.equals(retCode)
				||SFConst.RESPCODE_SUCCCODE_ZLSECU.equals(retCode)||"M0001".equals(retCode)){
			if(SFUtil.isEmpty(retCode)||SFConst.RESPCODE_SUCCCODE.equals(retCode)){
			...
			context.setDataValue(tranCode + "_O"+".RESP_MSG", SFUtil.isEmpty(retMsg)?SFConst.RESPCODE_SUCCMSG:BizUtil.returnErrMsg(context,"ZL", retMsg) );
		}else{
			...
			if(errorMap!=null){
				...
				context.setDataValue(tranCode + "_O"+".RESP_CODE", retCode);
				context.setDataValue(tranCode + "_O"+".RESP_MSG", BizUtil.returnErrMsg(context,"ZL", retMsg) );
			}
		}
		...
		if(secCompData!=null){
			...
			//从缓存中获取接口编码
			String secEncoding=ParamCache.getValue2("SEC_ZL_CHARSET", secCompCode+"_"+this.serviceName);
...
//			6022   6031  6032 6027
		...
		if(context.containsKey("ZLSECU_REQUEST_HEAD")){
			...
			if(SFUtil.isEmpty(txSeqId)){
				...
				// 修订txSeqId 长度问题，txSeqId 固定为8位不足8位左补0 -edit by lch 20180224
			...
			// 修订SUBCENTERID 为空问题，券商端交易SUBCENTERID 固定为0110 -edit by lch 20180224
			rootHeadColl.setDataValue("SUBCENTERID", "0110" );
			
			if("6065".equals(this.serviceName)){
				if(SFConst.SECU_GUANGDAZQ.equals(secCompCode)){ //光大
			...
			// 销户结息NODEID 为"0110800000000000",CHANNEL 为"0005" -edit by lch 20180224
			if("6045".equals(this.serviceName)){
				rootHeadColl.setDataValue("NODEID", "0110800000000000");
				rootHeadColl.setDataValue("CHANNEL", "0005");
			}
		}else{
