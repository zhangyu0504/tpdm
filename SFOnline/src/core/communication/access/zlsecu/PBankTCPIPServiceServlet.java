	...
	private void doInit() throws ServletException {
		...
		try {
			if (empIniFileName != null) {
				...
				SFLogger.info(SFConst.DEFAULT_TRXCODE, null,"Initialize EMP Flow Context from " + empIniFileName);
			...
			SFLogger.error(SFConst.DEFAULT_TRXCODE, null,"Initialize EMP Request Servlet from " + empIniFileName	+ " failed!", e);
...
//		}
	...
	private void initTCPIPAccessContext() {
		...
		try {
			...
			SFLogger.info(SFConst.DEFAULT_TRXCODE, null,
					"Initialize EMP Servlet Context from " + servletContextFile	+ " OK!");
		} catch (Exception e) {
	...
	public byte[] processNewPackage(byte[] msg, TCPIPService service,
		...
		try {
			SFLogger.info(SFConst.DEFAULT_TRXCODE, null,"Accept new request from: "+ socket.getRemoteSocketAddress().toString());
			if (requestHandler == null)
				throw new EMPException("TCPIPRequest handler not set for EMPTCPIPServiceServlet!");
			...
			request = requestHandler.getTCPIPRequest(msg, service, socket);
			...
			if (this.accessManager != null) {
				if(sessionContext != null)
					accessObj = accessManager.checkAccess(sessionContext, request,svcId);
				else{
					Context rootContext = this.componentFactory.getContextNamed(this.componentFactory.getRootContextName());
					accessObj = accessManager.checkAccess(rootContext, request,svcId);
				}
				if (accessObj != null)
			...
			if(secCompData!=null){
				...
				if(SFConst.SECU_GUOXINZQ.equals(secCompCode)||SFConst.SECU_GUOXINXY.equals(secCompCode)){ //国信 
					if("6065".equals(svcId)){
						//String txseqId = (String)context.getDataValue("ZLSECU_RESPONSE_HEAD.TXSEQID");
						//String resData ="1.0        6027 000 M 0        0        611      0        N                                                    <?xml version=\"1.0\"?><Root><Head><Version>01</Version><TxCode>6027</TxCode><FuncCode>100</FuncCode><Channel>0002</Channel><SubCenterId>0110</SubCenterId><NodeId>0110000000010008</NodeId><TellerId>000000</TellerId><TxSeqId>100016</TxSeqId><TxDate>20171127</TxDate><TxTime>170120</TxTime><UserId>0110000000010008</UserId></Head><Body><RespCode>M0000</RespCode><RespMsg>交易成功</RespMsg><CapAcct>110000885903</CapAcct><SecCompCode>10290000</SecCompCode><SecSeqId>100016</SecSeqId><AcctId>6230580000131099801</AcctId><RMBCurBal>10009999000998</RMBCurBal><HKDCurBal></HKDCurBal><USDCurBal></USDCurBal></Body></Root>";
						//retMsg=resData.getBytes();
						byte repChar=32;
						byte padChar=(byte)0x00;
			...
			if(SFConst.RET_RESP_YES.equals( SFUtil.getDataValue(context, SFConst.PUBLIC_IS_RET_RESP ))){
				//SFLogger.info(context, "直联券商应答报文(16进制)：["+DESUtil.bytesToHex(retMsg)+"]");
				
				// 返回请求结果
				if(retMsg != null){
					retMsg = service.send(retMsg, socket);
					SFLogger.info(context, "直联券商应答报文：["+ new String(retMsg) + "]");
				}
			}
			...
			e.printStackTrace();
			Context context = (Context) request.getAttribute(EMPConstance.ATTR_CONTEXT);
			//SFLogger.error(context,"service failed to handle TCPIP request!", e);
			...
			SFLogger.info(context, "直联券商返回异常响应报文：["+ new String(retMsg) + "]");
			if(retMsg != null){
				try {
					service.send(retMsg, socket);
				} catch (Exception ee) {
					SFLogger.error(context, "service failed!"+ ee);
				}
			}
		} finally {
