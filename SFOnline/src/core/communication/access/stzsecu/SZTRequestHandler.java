	...
	public void updateModel(byte[] requestMsg, Context context,
		...
		try {
			if (dataElementDef == null) {
				...
				String keyInputName = tcpipService.get("keyInputName");
				/**
				 * 设置公共上下文值
				 */
				//生成定长ID（系统初始流水号16位）
				...
				//keyInputName
				SFUtil.setDataValue(context, SFConst.PUBLIC_KEY_INPUTNAME, keyInputName);
				//发送响应报文标识
				...
				//券商类型
				SFUtil.setDataValue( context, SFConst.PUBLIC_SECU_TYPE, SFConst.SECU_SZT );
...
//			
			...
			if(SFConst.RESPCODE_SUCCCODE.equals(retCode)||SFUtil.isEmpty(retCode)){ 
				...
				SFUtil.setDataValue(context,rstKcoll,"RESP_MSG",SFUtil.isEmpty(retMsg)?SFConst.RESPCODE_SUCCMSG:BizUtil.returnErrMsg(context,"SZT", retMsg));
...
//					}
					SFUtil.setDataValue(context,rstKcoll,"RESP_CODE",errorMap.getSztCode());
					SFUtil.setDataValue( context, rstKcoll, "RESP_MSG", SFUtil.isEmpty( retMsg ) ? errorMap.getErrExplain() : BizUtil.returnErrMsg(context,"SZT", retMsg) );
				}else{
//					if(SFUtil.isEmpty(retMsg)){
//						retMsg = "交易失败";
//					}					
					SFUtil.setDataValue(context,rstKcoll,"RESP_CODE",retCode);
					SFUtil.setDataValue(context,rstKcoll,"RESP_MSG",SFUtil.isEmpty( retMsg ) ? "交易失败" : BizUtil.returnErrMsg(context,"SZT", retMsg));
				}
...
//			String tranCode = SFUtil.getDataValue(context,SFConst.PUBLIC_TX_CODE);
			...
			if(e instanceof SFException){
				...
				if(errorMap!=null){
					SFUtil.setDataValue(context,rstKcoll,"RESP_CODE",errorMap.getSztCode());
					SFUtil.setDataValue(context,rstKcoll,"RESP_MSG",SFUtil.isEmpty(errMsg)? errorMap.getErrExplain():BizUtil.returnErrMsg(context,"SZT", errMsg));
				}else{
					SFUtil.setDataValue(context,rstKcoll,"RESP_CODE",retCode);
					SFUtil.setDataValue(context,rstKcoll,"RESP_MSG",SFUtil.isEmpty(errMsg)? "交易失败":BizUtil.returnErrMsg(context,"SZT", errMsg));
				}
			...
			SFLogger.error( SFConst.DEFAULT_TRXCODE, null, "获得异常返回报文体失败 ",e1);
		}
	...
	private String send(Context context,String sendData) throws EMPException{
		...
		String destAppId = sztParam.getServerDestAppId();
		String destUserId = sztParam.getServerDestUserId();
		 ...
		 *  <br>
		 *  对应响应报文时Mr2Send方法:
		 *  1、SourceUserId=配置的ServerDestAppId
		 *  2、SourceAppId=配置的ServerDestUserId
		 *  3、DestUserId=原请求报文中的SourceUserId
		 *  4、DestAppId=原请求报文中的SourceAppId
		 */
		String pkgId = mrapi.Mr2Send( input, destUserId, destAppId, sourceUserId, sourceAppId, "", sourcePkId, "", "", "", ( byte )0, ( byte )0, ( byte )0, ( byte )0, 2000 );
		// 如果pkgid字符串为空，代表发送失败。
		if( pkgId == null ) {
			SFLogger.info( context, "响应失败：调用深证通 Mr2Send方法失败!" );
			throw new EMPException( "响应失败：调用深证通 Mr2Send方法失败!" );
		}
