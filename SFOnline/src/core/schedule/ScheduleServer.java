...
public class ScheduleServer {
	...
	private ScheduleServer()  {
	    try{
	    	...
	    	logger.error("new schedule server error:" + e.getMessage(), e);
		}
	}
	...
	public void startServer(ContextData c){
		try {	
			...
			logger.debug("Scheduler Server Start........");
		} catch (Exception e) {
			logger.error("Start Scheduler Server failed:" + e.getMessage(), e);
		}
	}
	...
	public void stopServer(boolean waitForJobsToComplete){
		try {
			...
			logger.debug("Scheduler Server stoped.");
		} catch (Exception e) {
			logger.error("Stop Scheduler Server failed." + e.getMessage(), e);
		}
	}
		...
		try {
			...
			for(ScheduleParam schedule:listScheduleParam){
			    ...
			    if (!ScheduleTaskInfo.SCHEDULE_TYPE_Forbidden.equals(schTaskType)){
			    	logger.info("添加调度任务:" + taskInfo.getName());
	...
	public void startScheduleTask(ScheduleTaskInfo schTaskInfo) throws SFException{
		try {		
		    ...
		    logger.debug("===triggerState==="+triggerState + " "+trigger.getName() + "  "+trigger.getGroup());
		    
		    debugSchedInfo();
		} catch (Exception e) {
			logger.error("addSchedTask fail:" + e.getMessage(), e);
			throw new SFException(e);
		}
	}	
	...
	public void stopScheduleTask(ScheduleTaskInfo schTaskInfo) throws SFException{
		try {
		    ...
		    logger.debug("===triggerState==="+triggerState + " "+schTaskName + "  ");
		    
		} catch (Exception e) {
			logger.error("stopScheduleTask fail:" + e.getMessage(), e);
			throw new SFException(e);
		}
	}	
	...
	public int getScheduleState(String schTaskName) throws SchedulerException{
	    ...
	    logger.debug("---getScheduleState--->"+schTaskName + " state:"+state);
	    return state;
	}
	...
	public void debugSchedInfo(){
		try {
			...
			for (int i = 0; i < jobDetailNames.length; i++) {
				logger.debug(i+" jobDetailName:" + jobDetailNames[i]);
			}
			...
			for (int i = 0; i < triggerNames.length; i++) {
				...
				logger.debug(i+" triggerName:" + triggerNames[i]  + " triggerState:" +triggerState);
			} 
		} catch (Exception e) {
		    logger.error("debugSchedInfo error:" + e.getMessage(), e);
		}
	}
